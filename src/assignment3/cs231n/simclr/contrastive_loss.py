...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2 * N):
            if j != k:
                denom_k = denom_k + torch.exp(sim(z_k, out[j]) / tau)
            if j != k + N:
                denom_k_N = denom_k_N + torch.exp(sim(z_k_N, out[j]) / tau)

        total_loss = total_loss - torch.log(
            torch.exp(sim(z_k, z_k_N) / tau) / denom_k
        )
        total_loss = total_loss - torch.log(
            torch.exp(sim(z_k_N, z_k) / tau) / denom_k_N
        )
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    positive = sim_positive_pairs(out_left, out_right)
    positive = torch.cat((positive, positive), dim=0)
