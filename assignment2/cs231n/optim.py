...
def sgd_momentum(w, dw, config=None):
    ...
    # in plain sgd, we just weighted the gradients to modify the weights

    # in sgd with momentum, we allow velocity to 'build up'

    # $mu$ is a hyperparameter. Between 0 and 1.
    # we will use the momentum that is given in config as a weight
...
def adam(w, dw, config=None):
    ...
    # rms prop like - update second moment
    config["v"] = config["beta2"] * config["v"] + \
                (1 - config["beta2"]) * (dw ** 2)
    
    # modifying t _before using it                        
    config['t'] += 1
