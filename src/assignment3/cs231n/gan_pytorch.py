...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_targets = torch.ones(logits_real.shape[0]).type(dtype)
    fake_targets = torch.zeros(logits_fake.shape[0]).type(dtype)
    loss = bce_loss(logits_real, real_targets) + bce_loss(logits_fake, fake_targets)
